...
export default function DeleteBannerModal({
  ...
  async function handleDelete() {
    ...
    setError('');
    try {
      await clientAxios.delete(`/api/admin/banners/${banner.id}`);
      onDeleted();
    } catch (err) {
  ...
  return (
    <Modal title="Xóa banner" onClose={onClose}>
      <div className="space-y-4">
        {error && <p className="text-danger text-sm">{error}</p>}
        <p className="text-sm">
          Bạn có chắc muốn xóa banner <span className="font-bold">{banner.title}</span>?
        </p>
