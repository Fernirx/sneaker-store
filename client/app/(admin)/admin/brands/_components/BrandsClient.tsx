...
import { formatDate, type BrandRow, type PageData } from './types';
...
export default function BrandsClient({
  ...
  async function fetchData(page: number, f: Filters) {
    ...
    try {
      ...
      const { data } = await clientAxios.get(`/api/admin/brands?${params}`);
      setPageData(data);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="font-display font-black text-xl uppercase tracking-tight">Thương hiệu</h1>
        {isAdmin && (
          <button
            ...
            {pageData.data.length === 0 ? (
              ...
              pageData.data.map(brand => (
                <tr key={brand.id} className="border-b border-line-2 last:border-0 hover:bg-paper/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-muted text-xs">{formatDate(brand.createdAt)}</td>
                  {isAdmin && (
                    <td className="px-4 py-3">
      ...
      {pageData.meta.totalPages > 1 && (
        <div className="flex items-center justify-between text-sm">
          <span className="text-muted text-xs">{pageData.meta.totalElements} thương hiệu</span>
