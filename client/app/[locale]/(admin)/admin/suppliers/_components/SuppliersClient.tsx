...
import { formatDate, type SupplierRow, type PageData } from './types';
...
export default function SuppliersClient({
  ...
  async function fetchData(page: number, f: Filters) {
    ...
    try {
      ...
      const { data } = await clientAxios.get(`/api/admin/suppliers?${params}`);
      setPageData(data);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="font-display font-black text-xl uppercase tracking-tight">Nhà cung cấp</h1>
        {canWrite && (
          <button
      ...
      {pageData.meta.totalPages > 1 && (
        <div className="flex items-center justify-between text-sm">
          <span className="text-muted text-xs">{pageData.meta.totalElements} nhà cung cấp</span>
