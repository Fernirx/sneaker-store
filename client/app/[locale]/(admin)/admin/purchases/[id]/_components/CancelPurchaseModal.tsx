...
export default function CancelPurchaseModal({
  ...
  purchaseId: number;
  onClose: () => void;
  onCancelled: (purchase: PurchaseResponse) => void;
}) {
  ...
  async function handleConfirm() {
    ...
    setError('');
    try {
      const { data } = await clientAxios.patch(`/api/admin/purchases/${purchaseId}/cancel`, {
  ...
  return (
    <Modal title="Hủy phiếu nhập hàng" onClose={onClose}>
      <div className="space-y-4">
        {error && <p className="text-danger text-sm">{error}</p>}
        <p className="text-sm">Bạn có chắc muốn hủy phiếu nhập hàng này?</p>
        <div>
