...
import { formatDateTime, STATUS_COLORS, type OrderResponse, type OrderStatus, type PageMeta } from './types';
...
export default function OrdersClient({ isLoggedIn }: { isLoggedIn: boolean }) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      <h1 className="font-display font-black text-3xl uppercase tracking-tight mb-8">{t('title')}</h1>
        ...
        {orders.map(order => (
          ...
          >
            ...
            <div className="flex items-center justify-between text-[12px] text-muted">
              <span>{formatDateTime(order.createdAt)} · {t('totalItems', { count: order.items.length })}</span>
